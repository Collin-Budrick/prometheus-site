...
- **Capacitor device host:** `PROMETHEUS_DEVICE_HOST` (IP or `127.0.0.1` for `adb reverse`, default `192.168.1.138` in dev/preview; set to `off` to disable) + optional `PROMETHEUS_DEVICE_WEB_PORT` (default `4173`) + `PROMETHEUS_DEVICE_PROTOCOL` (default `http`) override the live host in dev/preview; WebTransport flags default off in this mode unless explicitly set.
