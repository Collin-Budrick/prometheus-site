...
.store-stream-sort-trigger:focus {
  outline: none;
}

.store-stream-sort-caret {
  display: inline-flex;
  transform: translateY(-1px);
  transition: transform 180ms ease;
}

.store-stream-sort-menu[data-open='true'] .store-stream-sort-caret {
  transform: rotate(180deg) translateY(1px);
}

.store-stream-sort-drawer {
  position: absolute;
  top: calc(100% + 10px);
  right: 0;
  display: grid;
...
.store-stream-sort-option:hover {
  ...
  box-shadow: 0 10px 18px rgb(var(--accent) / 0.12);
}

.store-stream-sort-option[data-active='true'] {
  color: rgb(var(--ink));
