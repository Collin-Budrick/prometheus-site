...
import type {
  ...
  HomeSharedRuntimeConnection,
  HomeSharedRuntimeRequestOptions
...
export const readStaticHomeWidthBucketHint = (
  ...
  const layout = parseFragmentHeightLayout(card.getAttribute('data-fragment-height-layout'))
  if (!layout) {
    return null
  }
...
export const connectHomeAnchorSharedRuntime = ({
  controller,
  root = document,
  runtimePlanEntries,
  runtimeFetchGroups,
  ...
  const viewportWidth =
    typeof window.innerWidth === 'number' && window.innerWidth > 0
      ? window.innerWidth
      : 1280
  const initialSizing = collectStaticHomeSizingSeeds(fragmentOrder, viewportWidth, root)
  ...
  sharedRuntime.attachHandlers({
    ...
    onStatus: (status) => {
      updateFragmentStatus(controller.lang, status === 'idle' ? 'idle' : 'streaming')
    },
    onError: (message) => {
  ...
  document.addEventListener('prom:fragment-stable-height', handleStableHeight as EventListener)
  controller.cleanupFns.push(() =>
    document.removeEventListener('prom:fragment-stable-height', handleStableHeight as EventListener)
  )
  controller.cleanupFns.push(() => {
...
export const scheduleStaticHomePaintReady = ({
  ...
  scheduleStaticRoutePaintReady({
    root: resolveStaticHomePaintRoot(root),
...
const collectPendingHomeFragmentCards = (root: ParentNode = document): HydratableHomeFragmentCard[] =>
  Array.from(root.querySelectorAll<HTMLElement>(`[${STATIC_FRAGMENT_CARD_ATTR}]`)).flatMap((card) => {
...
export const bindHomeAnchorFragmentHydration = ({
  controller,
  root = document,
  requestFragments,
  ...
  controller: Pick<HomeControllerState, 'destroyed' | 'lang' | 'patchQueue' | 'fetchAbort'>
  root?: ParentNode
  requestFragments?: (ids: string[], options: HomeSharedRuntimeRequestOptions) => Promise<void>
  ...
  const collectQueuedIds = (stage: Extract<StaticHomeCardStage, 'anchor' | 'deferred'>) => {
    ...
    return Array.from(root.querySelectorAll<HTMLElement>(`[${STATIC_FRAGMENT_CARD_ATTR}]`))
      .flatMap((card) => {
        const id = card.dataset.fragmentId
  ...
  const runHydrationBatch = async () => {
    if (controller.destroyed || !requestFragments) return
    ...
    try {
      await requestFragments(ids, { isAnchorBatch })
      if (controller.destroyed || controller.fetchAbort !== fetchAbort || fetchAbort.signal.aborted) {
        return
      }
  ...
  const manager: HomeFragmentHydrationManager = {
    observeWithin(nextRoot) {
      if (controller.destroyed) return
      Array.from(nextRoot.querySelectorAll<HTMLElement>(`[${STATIC_FRAGMENT_CARD_ATTR}]`))
        .flatMap((card) => {
          const id = card.dataset.fragmentId
          const stage = card.getAttribute(STATIC_HOME_STAGE_ATTR)
