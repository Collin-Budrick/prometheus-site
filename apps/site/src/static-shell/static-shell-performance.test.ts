...
describe('static shell performance invariants', () => {
  it('keeps the home bootstrap on the fast path', async () => {
    ...
    expect(bootstrapSource).toContain('scheduleHomePreviewRefresh(controller, homeFragmentHydration)')
    ...
    expect(globalCriticalSource).toContain("> .fragment-card:not([data-static-home-fragment-kind='manifest'])")
  ...
  it('preloads the static shell bootstrap and avoids split entry builds', async () => {
    ...
    expect(homeStaticEntrySource).toContain('HOME_BOOTSTRAP_POST_LOAD_DELAY_MS')
    ...
    expect(homeStaticEntrySource).toContain("window.addEventListener('load', loadHandler, { once: true })")
