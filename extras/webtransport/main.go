...
func buildStreamURL(apiBase, path, lang, protocol, known string) string {
	if path == "" {
		path = "/"
	}
	values := url.Values{}
...
func newWebTransportServer(cfg config, handler http.Handler) *webtransport.Server {
...
func main() {
	...
	server := newWebTransportServer(cfg, mux)
...
func handleSession(session *webtransport.Session, r *http.Request, cfg config) {
	...
	req, err := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		buildStreamURL(cfg.apiBase, path, lang, protocol, known),
		nil,
	)
	if err != nil {
...
func isHeartbeatFrame(frame []byte) bool {
	return len(frame) == 8 &&
		binary.LittleEndian.Uint32(frame[0:4]) == 0 &&
		binary.LittleEndian.Uint32(frame[4:8]) == 0
}

func shouldSendFrameAsDatagram(frame []byte, maxDatagramSize int) bool {
	return maxDatagramSize > 0 && len(frame) <= maxDatagramSize && !isHeartbeatFrame(frame)
}
...
func relayFragmentDatagrams(
	...
	for {
		...
		for {
			...
			sent := false
			if shouldSendFrameAsDatagram(frame, maxDatagramSize) {
