...
export const ensureCaddyConfig = (override?: string, prodOverride?: string, options: CaddyConfigOptions = {}) => {
  ...
  const allowedOriginPattern = [domains.web, domains.webProd].map(escapeRegex).join('|')
  ...
  const buildSite = (host: string, upstream: string, siteOptions: SiteCaddyConfigOptions) => {
    ...
    const staticBlock = siteOptions.servePrecompressed
      ? `\thandle /build/* {\n\t\theader Cache-Control "public, max-age=31536000, immutable"\n\t\troot * ${staticRoot}\n\t\tfile_server {\n\t\t\tprecompressed br gzip\n\t\t}\n\t}\n\n\thandle /assets/* {\n\t\theader Cache-Control "public, max-age=31536000, immutable"\n\t\troot * ${staticRoot}\n\t\tfile_server {\n\t\t\tprecompressed br gzip\n\t\t}\n\t}\n\n\t@static {\n\t\tpath_regexp static_files ^/(?:icons/|favicon\\.[^/]+$|manifest\\.webmanifest$|service-worker\\.js$|q-manifest\\.json$|robots\\.txt$|sitemap\\.xml$)\n\t\tfile {\n\t\t\troot ${staticRoot}\n\t\t\ttry_files {path} {path}.br {path}.gz\n\t\t}\n\t}\n\thandle @static {\n\t\theader Cache-Control "public, max-age=31536000, immutable"\n\t\troot * ${staticRoot}\n\t\tfile_server {\n\t\t\tprecompressed br gzip\n\t\t}\n\t}\n\n`
      : ''
  ...
  const buildSpacetimeDbSite = (host: string) =>
    `${host} {\n\ttls ${caddy.certPemPath} ${caddy.certKeyPath}\n\theader {\n\t\talt-svc "h3=\\":443\\"; ma=2592000"\n\t\tStrict-Transport-Security "max-age=31536000; includeSubDomains; preload"\n\t}\n\n\t@allowed_origin header_regexp Origin ^https://(?:${allowedOriginPattern})(?::\\d+)?$\n\theader @allowed_origin {\n\t\tAccess-Control-Allow-Origin "{http.request.header.Origin}"\n\t\tAccess-Control-Allow-Headers "*"\n\t\tAccess-Control-Allow-Methods "GET, POST, PUT, PATCH, DELETE, OPTIONS"\n\t\tAccess-Control-Expose-Headers "*"\n\t\tVary "Origin"\n\t}\n\n\t@preflight {\n\t\tmethod OPTIONS\n\t\theader_regexp Origin ^https://(?:${allowedOriginPattern})(?::\\d+)?$\n\t}\n\trespond @preflight 204\n\n\thandle {\n\t\treverse_proxy http://spacetimedb:3000 {\n\t\t\tlb_try_duration 5s\n\t\t\tlb_try_interval 100ms\n\t\t}\n\t}\n}\n`
