...
const GENERATED_MARKER_EXEMPTIONS = new Set([
...
const hasGeneratedMarker = (relativePath: string) => {
  if (GENERATED_MARKER_EXEMPTIONS.has(relativePath)) return true
  const absolutePath = path.join(root, relativePath)
...
switch (command) {
  case 'init':
    ...
    break
  case 'new':
    await importScriptWithArgs('./template-new.ts', process.argv.slice(3))
    break
  case 'sync':
    await importScriptWithArgs('./template-sync.ts', process.argv.slice(3))
    break
  case 'doctor':
    await importScriptWithArgs('./template-doctor.ts', process.argv.slice(3))
    break
  case 'diff':
    await importScriptWithArgs('./template-diff.ts', process.argv.slice(3))
    break
  case 'report':
    await importScriptWithArgs('./template-report.ts', process.argv.slice(3))
    break
  case 'upgrade':
    await importScriptWithArgs('./template-upgrade.ts', process.argv.slice(3))
    break
  case 'create-feature':
    await importScriptWithArgs('./template-create-feature.ts', process.argv.slice(3))
    break
  case 'check':
  ...
  default:
    throw new Error(
      `[template] Unknown command '${command}'. Expected init, new, sync, doctor, diff, report, upgrade, create-feature, or check.`
    )
}
